...
const Sidebar = ({ isOpen, onClose }) => {
  ...
  const menuItems = [
    ...
    { label: 'About', icon: User, path: '/admin/dashboard/about', permission: 'dashboard' },
    { label: 'Projects', icon: Briefcase, path: '/admin/dashboard/projects', permission: 'projects' },
    { label: 'Certificates', icon: Award, path: '/admin/dashboard/certificates', permission: 'certificates' },
    { label: 'Skills', icon: Code, path: '/admin/dashboard/skills', permission: 'skills' },
    { label: 'Services', icon: FileText, path: '/admin/dashboard/services', permission: 'services' },
    { label: 'Testimonials', icon: Quote, path: '/admin/dashboard/testimonials', permission: 'testimonials' },
    { label: 'Messages', icon: MessageSquare, path: '/admin/dashboard/messages', permission: 'messages' },
    { label: 'Settings', icon: Settings, path: '/admin/dashboard/settings', permission: 'settings' },
  ...
  return (
    <>
      ...
      <aside className={`fixed lg:sticky top-0 left-0 w-64 border-r flex flex-col h-screen z-[70] transition-all duration-300 lg:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'} ${isDark ? 'bg-[#0a0a0f] border-gray-800' : 'bg-bg-card border-border-theme shadow-md'
        }`}>
